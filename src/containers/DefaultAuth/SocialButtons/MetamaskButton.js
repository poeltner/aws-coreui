  ...
  return class extends Component {
    ...
    signIn() {
      ...
      web3.eth.getAccounts((error, accounts) => {
        if (accounts.length == 0) {
          ...
          web3.personal.sign(
            web3.fromUtf8(
              `I am signing my one-time nonce: ` + this.state.nonce
            ),
            accounts[0],
            (err, signature) => {
              ...
              fetch(
                this.props.serverUrl +
                  "/authorization?publicAddress=" +
                  accounts[0] +
                  "&response_type=id_token&signature=" +
                  signature
              )
                .then(response => {
    ...
    federatedSignIn(response) {
      ...
      Auth.federatedSignIn(
        process.env.REACT_APP_AWS_SIGNIN_PROVIDER,
    ...
    render() {
      ...
      return <Comp {...this.props} metamaskSignIn={this.signIn} />;
    }
  };
}

const CustomButton = props => (
  <Button className="btn" onClick={props.metamaskSignIn} block>
